...
case "$2" in
  "") echo "Usage: run.sh deploy|diagnose|report|repair|update|backup|(other ansible tags...) development|provisioning|debug|maintenance|tunnel [machine] [output_directory]";;
  *) 
    case "$3" in
  	  "") if [[ $OSTYPE == darwin* ]]; 
